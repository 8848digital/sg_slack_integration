	...
	refresh(frm) {
		...
		if (enabled == 1) {
			cur_frm.add_custom_button(__("E-Way Bill"), function () {
				frappe.call({
					method: "cleartax_integration.cleartax_integration.API.ewb.shipment_ewb",
					args: {
				...
				cur_frm.add_custom_button(__("Update PARTB"), function () {
					let d1 = new frappe.ui.Dialog({
						...
						primary_action(values) {
							...
							frappe.call({
								method: "cleartax_integration.cleartax_integration.API.ewb.update_ewb_partb_sh",
								args: {
									data: values,
			...
			cur_frm.add_custom_button(__("Cancel E-Way Bill"), function () {
				let d = new frappe.ui.Dialog({
					...
					primary_action(values) {
						...
						frappe.call({
							method: "cleartax_integration.cleartax_integration.API.ewb.cancel_ewb_sh",
							args: {
								data: values,
