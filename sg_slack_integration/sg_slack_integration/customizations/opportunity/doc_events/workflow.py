...
def handle_workflow_state(self):
	...
	if channel:
		...
		if opportunity_exist:
			...
			if (
				previous_workflow == "Submit To BD"
				and self.workflow_state == "Client Approval Pending"
			):
			...
			elif (previous_workflow == "Client Approval Pending"
         		and self.workflow_state == "In Progress"
...
def set_archiving_date(self):
    ...
    self.custom_archiving_date = date.today() + timedelta(days = days_to_archive)